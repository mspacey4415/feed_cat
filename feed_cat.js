...
<head>
    ...
    <style>
        ...
        .cat, .kid {
            ...
            min-width: 60px; /* NEW: Ensure kid always has a measurable width */
        }
...
<body>
    ...
    <script>
        ...
        const windFillTime = 5000; // Time in milliseconds for the wind gauge to fill (5 seconds)
        const windDuration = 2000; // Time in milliseconds the wind lasts once active (2 seconds)
        const kidMoveSpeed = 20; // Pixels the kid moves per button click
        const windPullSpeed = 5; // Pixels the kid is pulled left per frame during wind
        const winCooldownTime = 500; // Time in milliseconds after "Ouch!" before win is possible
        ...
        function startWindGaugeTimer() {
            ...
            windGaugeInterval = setInterval(() => {
                ...
                windGaugeProgress += (100 / (windFillTime / 1000));
        ...
        function deactivateWind() {
            ...
            cat.textContent = '🐱'; // Reset cat emoji
            // Wind gauge timer will be restarted by the "Ouch!" callback or game loop
        }
                    ...
                    if (isWindActive) {
                        ...
                        if (kidHP <= 0) {
                            ...
                            showGameDialog("Ouch!", "ouch", () => {
                                ...
                                deactivateWind(); // Stop wind and reset cat emoji
                                startWindGaugeTimer(); // Restart wind gauge
                                ...
                                }, winCooldownTime); // Wait for cooldown before resuming
                            });
                        }
                    } else {
